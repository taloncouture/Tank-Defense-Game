...
class Enemy(pygame.sprite.Sprite):
    def __init__(self, x, y, width, height, speed, type, shoot_delay, bullet_speed):
        ...
        # self.enemy_right = pygame.transform.rotate(self.image, -90)
        # self.enemy_left = pygame.transform.rotate(self.image, 90)
        # self.enemy_up = pygame.transform.rotate(self.image, 0)
        # self.enemy_down = pygame.transform.rotate(self.image, 180)
    ...
    def move(self):
        # if self.rect.x < player.rect.x and abs(player.rect.x - self.rect.x) >= abs(player.rect.y - self.rect.y):
            ...
            # if self.rect.y > player.rect.y and abs(player.rect.y - self.rect.y) > abs(player.rect.x - self.rect.x):
...
def obstacle_collide(sprite):
    if pygame.sprite.spritecollide(sprite, flag_group, False) or pygame.sprite.spritecollide(sprite, obstacle_group, False):
        return True
    else:
        return False
...
def create_obstacles(x, y):

    obstacle = Obstacle(tile_width * x, tile_height * y, obstacle_width, obstacle_height)
...
while True:
        ...
        if game_active:
            #print(spawn_status)
            #spawn_enemies(difficulty_modifier, spawn_status, spawn_time)
